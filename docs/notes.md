...
| --- | --- | --- | --- | --- | --- |
| 1920×1080 | 4 + 1 | 458 px | 534 px | 412 px | 无 |
| 1600×1050 | 3 + 2 | 509 px | 516 px | 394 px | 无 |
| 1440×900 | 3 + 2 | 456 px | 426 px | 304 px | 无 |
| 1280×800 | 2 + 2 + 1 | 612 px | 366 px | 197 px | 无（卡内竖滚） |
| 390×844（手机） | 单列长页 | 351 px | — | — | **1 条**：顶栏 chips |

模块卡的最小宽度是 `--card-min: 410px`，`auto-fit` 放不下就换行 ——
卡数变了不用再改列数。代价说清楚：**1280 那档会折成 3 行**，
每行只剩约 100px，四张卡各自内部滚，画面被压到 197px 高；
1440 以上看不出问题，反而因为卡片变宽干掉了原来目标表和使用说明卡上的两条横向滚动条。
再往下要救 1280，就在 `@media (max-width: 1400px)` 里把 `--card-min` 降回 320px。

右栏宽度跟着模块卡走（`syncRailWidth()` 量真实卡片宽度写回 `--rail-w`），
但**封顶 34% 视口宽**：不然 1280 那档卡片 612px 宽，右栏跟着变 612，画面直接挤没。
