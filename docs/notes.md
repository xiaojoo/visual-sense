...
**操作日志卡的下限 = 性能指标卡的高度**（`syncLayout()` 量真实渲染写进 `--metrics-h`，
CSS 一句 `.rail .card.grow { min-height: var(--metrics-h, auto) }`）。
14 条日志时实测日志卡：改之前 2560→156px、1920→116、1600→98、1440→79（都比指标卡 178.73 矮），
改之后五档全部 179px，和指标卡齐平。
**代价要写清楚，而且它不取决于日志有多少条**：日志卡塌不下去之后右栏就装不下了 ——
实测 3 条、8 条、14 条日志三种情况下，2560/1920/1600/1440/1280 **五档全部竖滚**
（1920 那档内容 187+179+179+24 = 569px vs 右栏只有 510px，超 59px）；
改之前只有 1440/1280 两档滚，1920/1600 是不滚的。
`check_layout` 里对着这条的断言是"日志卡高度必须 ≥ 指标卡高度（容差 2px）"，
反证做过：把 `min-height` 改回 `auto`，立刻报 4 条。
两个版本各自剩下的毛病摆在这里，选一个：V1 日志能塌到 79~116px（比指标卡矮一截，他不满的就是这个）；
V2 两卡齐平但右栏每档都要滚。要回 V1 就把 `min-height` 改回 `auto` 并删掉 `--metrics-h` 那段。
